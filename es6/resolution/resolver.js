...
const _resolveFactoryFromBinding = (binding, context) => {
    const factoryDetails = binding_utils_1.getFactoryDetails(binding);
    return exceptions_1.tryAndThrowErrorIfStackOverflow(() => factoryDetails.factory.bind(binding)(context), () => new Error(ERROR_MSGS.CIRCULAR_DEPENDENCY_IN_FACTORY(factoryDetails.factoryType, context.currentRequest.serviceIdentifier.toString())));
};
...
const _resolveInScope = (requestScope, binding, resolveFromBinding) => {
    let result = scope_1.tryGetFromScope(requestScope, binding);
    if (result !== null) {
        return result;
    }
